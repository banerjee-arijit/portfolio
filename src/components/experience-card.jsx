...
export default function ExperienceCard({ experience }) {
  ...
  return (
    <div className="flex flex-col justify-between md:flex-row md:items-start border-b border-neutral-100 dark:border-neutral-900 pb-6 last:border-0 last:pb-0">
        ...
        <div className="my-4 flex flex-wrap gap-2">
          {experience.techStack.map((tech, index) => (
            ...
            >
              <Suspense
                fallback={
                  <div className="h-4 w-4 rounded bg-neutral-200/50 dark:bg-neutral-800/50 animate-pulse shrink-0" />
                }
              >
                <StackIcon
                  name={tech.iconName}
      ...
      <div className="hidden md:flex shrink-0 h-28 w-28 items-center justify-center relative group">
        <img
          src={experience.companyLogo}
          alt={experience.company}
          className="h-full w-full object-contain transition-transform duration-500 group-hover:scale-110 relative z-10 drop-shadow-sm"
        />
      </div>
    </div>
  );
